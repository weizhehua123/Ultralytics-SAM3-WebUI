...
_Exported on 02/03/2026 at 00:58:38 GMT+8 from Copilot Chat via WayLog_
...
我会先落地“进程内 job 系统”（队列、状态落盘、结果目录、worker 启动），让 `/api/v1/jobs/{id}` 能真实返回状态；然后再把图片/视频端点改为创建 job 并交给 worker
